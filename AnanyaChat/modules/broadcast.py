...
@AnanyaBot.on_message(filters.command("broadcast") & filters.user(OWNER_ID))
async def broadcast_(_, message: Message):
    ...
    for chat_id in recipients:
        try:
            msg = await (reply.copy(chat_id) if reply else AnanyaBot.send_message(chat_id, text=text))
